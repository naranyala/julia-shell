...
@testset "path safety and protocol" begin
    mktempdir() do root
        ...
        @test_throws DockyardError safe_target_path("/etc/passwd"; env=env)
        @test_throws DockyardError resolve_variables(raw"${PATH}"; env=env)
        ...
        @test_throws DockyardError decode_message("{\"v\":2}")
    end
end
...
@testset "safe deployment and conflict planning" begin
    mktempdir() do root
        ...
        snapshots_list = Dockyard.list_snapshots(root=snapshots)
        ...
        @test_throws DockyardError apply!(conflicting; repo=repo, env=env, yes=true, snapshot_root=snapshots)
...
@testset "snapshot restore and pins" begin
    mktempdir() do root
        ...
        @test_throws DockyardError pin!(repo, "org.example.Second.desktop"; env=env, expected_revision=0)
