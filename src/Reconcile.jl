...
function _state_file(repo::AbstractString="."; env=ENV)
    joinpath(juliashell_state_dir(; env), "profiles", _repository_key(repo), "state.toml")
end

"Serialize repository mutations across CLI processes and daemon connections."
function _with_mutation_lock(f, repo::AbstractString; env=ENV)
    Sys.isunix() || throw(JuliaShellError(:lock_unavailable, "repository mutation locking is only supported on Unix";
        remediation="run julia-shell on a Unix-like system"))
    lock_directory = dirname(_state_file(repo; env))
    mkpath(lock_directory)
    lock_path = joinpath(lock_directory, "mutation.lock")
    open(lock_path, "w") do io
        fd = Base._fd(io)
        while true
            acquired = ccall(:flock, Cint, (Base.RawFD, Cint), fd, _LOCK_EX | _LOCK_NB) == 0
            acquired && break
            errno = Base.Libc.errno()
            errno in (11, 35) || throw(JuliaShellError(:lock_unavailable, "could not acquire the repository mutation lock";
                details=Dict("path" => lock_path, "errno" => errno), remediation="retry after the other mutation finishes"))
            sleep(0.01)
        end
        try
            f()
        finally
            ccall(:flock, Cint, (Base.RawFD, Cint), fd, _LOCK_UN)
        end
    end
end
...
function plan(repo::AbstractString; profile="personal", env=ENV)
    ...
    isfile(path) || throw(JuliaShellError(:profile_missing, "profile does not exist";
                                        details=Dict("path" => path), remediation="run julia-shell init"))
...
function _journal_path(id::AbstractString; env=ENV)
    joinpath(juliashell_state_dir(; env), "journal", String(id) * ".toml")
end
...
function _stage_action(action::PlanAction, stage::String)
    ...
    action.mode == "generated" && throw(JuliaShellError(:unsupported_mode,
...
function _apply_unlocked!(current_plan::Plan; repo=".", env=ENV, yes=false, force=false,
                          snapshot_root=nothing)
    blocked = _plan_blocked(current_plan)
    blocked !== nothing && throw(JuliaShellError(:plan_blocked, blocked;
    ...
    yes || throw(JuliaShellError(:confirmation_required, "apply requires explicit confirmation";
    ...
    !force && any(_plan_has_changed, actions) && throw(JuliaShellError(:plan_stale,
    ...
    try
        ...
        for action in actions
            ...
            stage = action.target * ".julia-shell-stage-" * transaction_id * "-" * action.id
            backup = action.target * ".julia-shell-backup-" * transaction_id * "-" * action.id
            _entry_exists(stage) && _remove_entry(stage)
        ...
        for action in actions
            _target_matches(action) || throw(JuliaShellError(:verification_failed,
        ...
        try
            ...
            _write_journal(journal_path, journal)
            throw(JuliaShellError(:recovery_required, "transaction failed and rollback needs recovery";
                details=Dict("journal" => journal_path, "error" => sprint(showerror, err),
                             "rollback_error" => sprint(showerror, rollback_error)),
                remediation="keep the journal and run julia-shell doctor"))
        end
        err isa JuliaShellError && throw(err)
        throw(JuliaShellError(:transaction_failed, "transaction failed and was rolled back";
            details=Dict("journal" => journal_path, "error" => sprint(showerror, err)),
            remediation="inspect the journal and retry after resolving the reported error"))
    end
end

"Apply a plan through a serialized snapshot, stage, commit, and verification transaction."
function apply!(current_plan::Plan; repo=".", env=ENV, yes=false, force=false,
                snapshot_root=nothing)
    _with_mutation_lock(repo; env=env) do
        _apply_unlocked!(current_plan; repo=repo, env=env, yes=yes, force=force,
...
function recover_journals!(; env=ENV)
    directory = joinpath(juliashell_state_dir(; env), "journal")
...
function _mutate_profile_unlocked(transform, repo::AbstractString, profile_name::AbstractString, request_id;
    ...
    expected_revision !== nothing && Int(expected_revision) != current_revision &&
        throw(JuliaShellError(:revision_conflict, "profile revision is stale";
...
function _mutate_profile(transform, repo::AbstractString, profile_name::AbstractString, request_id;
                         expected_revision=nothing, env=ENV, operation)
    _with_mutation_lock(repo; env=env) do
        _mutate_profile_unlocked(transform, repo, profile_name, request_id;
                                 expected_revision=expected_revision, env=env, operation=operation)
    end
end
...
function pin!(repo::AbstractString, desktop_id::AbstractString; profile="personal", position=nothing,
    ...
    _mutate_profile(repo, profile, request_id; expected_revision, env, operation="pin") do current
        any(pin.desktop_id == desktop_id for pin in current.dock.pins) &&
            throw(JuliaShellError(:duplicate_pin, "application is already pinned";
...
function unpin!(repo::AbstractString, desktop_id::AbstractString; profile="personal",
    ...
    _mutate_profile(repo, profile, request_id; expected_revision, env, operation="unpin") do current
        ...
        index === nothing && throw(JuliaShellError(:pin_missing, "application is not pinned";
            details=Dict("desktop_id" => String(desktop_id)), remediation="inspect julia-shell status"))
        deleteat!(pins, index)
        Profile(current.name; schema=current.schema,
                dock=Dock(; edge=current.dock.edge, output=current.dock.output,
                           autohide=current.dock.autohide, pins=_renumber(pins)),
...
function reorder!(repo::AbstractString, from::Integer, to::Integer; profile="personal",
    ...
    _mutate_profile(repo, profile, request_id; expected_revision, env, operation="reorder") do current
        ...
        (1 <= from <= length(pins) && 1 <= to <= length(pins)) ||
            throw(JuliaShellError(:invalid_position, "reorder position is outside the pin list";
...
function _adopt_unlocked!(repo::AbstractString, existing::AbstractString; id=basename(existing), target=existing,
    ...
    _entry_exists(source_path) || throw(JuliaShellError(:source_missing, "adopt source does not exist";
        details=Dict("path" => source_path), remediation="choose an existing file or directory"))
    _likely_secret(source_path) && !allow_secret &&
        throw(JuliaShellError(:secret_excluded, "likely secret paths are excluded by default";
            details=Dict("path" => source_path), remediation="use a secret manager or explicitly pass --allow-secret"))
    mode in ("copy", "symlink") || throw(JuliaShellError(:invalid_mode, "adopt mode must be copy or symlink";
    ...
    is_path_within(destination, repository) || throw(JuliaShellError(:unsafe_path, "adopt id escapes the repository";
        remediation="use a simple logical ID"))
    is_path_within(source_path, destination) && throw(JuliaShellError(:invalid_adoption, "adopt source is already below the destination";
    ...
    any(entry.id == id for entry in current.dotfiles) && throw(JuliaShellError(:duplicate_entry, "dotfile id is already managed";
    ...
    ispath(destination) && throw(JuliaShellError(:destination_exists, "repository destination already exists";
    ...
    result = _mutate_profile_unlocked(repository, profile, request_id; expected_revision, env, operation="adopt") do loaded
...
"Import an existing user path under the repository mutation lock."
function adopt!(repo::AbstractString, existing::AbstractString; id=basename(existing), target=existing,
                profile="personal", mode="copy", allow_secret=false, env=ENV,
                expected_revision=nothing, request_id=string(uuid4()), snapshot_root=nothing)
    _with_mutation_lock(repo; env=env) do
        _adopt_unlocked!(repo, existing; id=id, target=target, profile=profile, mode=mode,
                         allow_secret=allow_secret, env=env, expected_revision=expected_revision,
                         request_id=request_id, snapshot_root=snapshot_root)
    end
end
...
function _restore_snapshot_unlocked(path::AbstractString; ids=String[], yes=false, env=ENV,
                                     snapshot_root=nothing, repo=".")
    yes || throw(JuliaShellError(:confirmation_required, "restore requires explicit confirmation";
    ...
    selected = isempty(ids) ? entries : begin
        ...
        isempty(unknown) || throw(JuliaShellError(:entry_missing, "snapshot selector does not match an entry";
    ...
    try
        ...
        try
            ...
            throw(JuliaShellError(:recovery_required, "restore failed and rollback needs recovery";
                details=Dict("journal" => journal_path, "error" => sprint(showerror, err),
                             "rollback_error" => sprint(showerror, rollback_error)),
                remediation="run doctor with the journal path"))
        end
        err isa JuliaShellError && throw(err)
        throw(JuliaShellError(:transaction_failed, "restore failed and was rolled back";
            details=Dict("journal" => journal_path), remediation="inspect the journal and retry"))
    end
end

"Restore a snapshot through a serialized repository mutation transaction."
function restore_snapshot(path::AbstractString; ids=String[], yes=false, env=ENV,
                          snapshot_root=nothing, repo=".")
    _with_mutation_lock(repo; env=env) do
        _restore_snapshot_unlocked(path; ids=ids, yes=yes, env=env,
                                    snapshot_root=snapshot_root, repo=repo)
    end
end
